...
interface AddDistrictRateModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function AddDistrictRateModal({ isOpen, onClose }: AddDistrictRateModalProps) {
  const { addDistrictRate } = useOrders();

  const [district, setDistrict] = useState(districts[0] || 'Palakkad');
  ...
  const [ratePerSqft, setRatePerSqft] = useState<number>(35);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to create district rate');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-md overflow-hidden rounded-lg border border-ink-700 bg-ink-950 shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-ink-800 px-5 py-4">
          <h3 className="text-base font-extrabold text-white">Add New District Rate</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            >
              {loading ? 'Creating...' : 'Add District Rate'}
            </button>
          </div>
        </form>
      </div>
